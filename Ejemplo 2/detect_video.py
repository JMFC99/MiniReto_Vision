...
def main(_argv):
    ...
    while True:
        ...
        if(classes[0][1]>=1):
            img,box_coords = draw_outputs(img, (boxes, scores, classes, nums), class_names)
            msj = ''
            if(box_coords):
                L1 = 80
                L2 = 80
                ...
                A = L1+L1*L2*math.cos(q2)
                B = L2*math.cos(q2)
                q1 = (y*B + x*A)/(B**2+A**2)
                for i,cord in enumerate(box_coords):
                    if(i==0):
                        msj = str(cord) +  msj
                    else:
                        msj = str(cord) + ',' +  msj
                msj =  msj + ',' + str(q2) +',' + str(q1) 
            else:
                msj = '1,1,1,1,1,1'
            ...
            # print(class_names.index('cell phone'))
            img = cv2.putText(img, "figure not in frame", (0, 30),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2)
        if FLAGS.output:
            out.write(img)
